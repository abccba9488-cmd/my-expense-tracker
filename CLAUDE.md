...
3. `_parse_disclosure()` 解析說明全文裡的自結合併財務資訊表格，只取**單月**資料（不再解析季/累計）：
   ...
   - B）TPEX「otc」三段式，**區段編號可能是阿拉伯數字 `(1)單月 (2)單季 (3)最近四季累計`，也可能是中文數字 `(一)單月 (二)單季`**（不同公司格式不同，兩種都要認），只看「單月」區段，EPS 列 3 個數字（本期/去年同期/年增%），**去年同月EPS 是表格直接給的，不用反推**
...
4. **無主旨 pre-filter，篩選依據是「有沒有解析出 `monthly_eps`」，不是單純比對說明欄是否含「每股盈餘」這四個字**——原本對齊參考實作（n8n）用的就是這種簡單字串比對，但「限制員工權利新股」「庫藏股」「可轉債」等公告依法都要寫一段「對公司每股盈餘稀釋情形」的揭露文字，會被誤判成自結公告。現在改成：先呼叫 `_parse_disclosure()`，只有真的解析出 `monthly_eps`，或主旨/說明含「注意交易資訊」（這類公告常常沒有財務表格，但仍刻意保留，見下一點），才算相關
