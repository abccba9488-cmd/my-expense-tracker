...
`ma20`/`ma60`/`ma120`/`ma240`：各自以相關子查詢取該股最近 20／60／120／240 筆 `daily_prices.close`（`ORDER BY date DESC LIMIT N`，吃 `ix_dp_code_date` 索引，不用整表掃描）算出的簡單移動平均。前端四個表格（主表格／飆股清單／自選股／達人選股列表）最後一欄「**甜蜜點**」皆呼叫 `app.js` 的 `sweetSpotCell(s)` 顯示此值——**紅＝ `ma20`、黃＝ `ma60`、綠＝ `ma120`、紫＝ `ma240`**。四條均線用同一套**刻意不對稱**的判定規則（不是 ±3%）：`(close - ma) / ma <= 0.03`，也就是股價只要在該均線之下（不論低多少都算），或高於均線但漲幅不超過 3%，就顯示對應顏色＋🔔 圖示提示；漲超過 3% 以上就不顯示——把每條均線都當成「長期價值區」而非單純的支撐/壓力測試，只有「跌破或剛站上」才算「甜蜜點」，漲多了就不算。`_SWEET_SPOT_TIERS` 陣列依序 20→60→120→240 檢查，同時符合多條時優先顯示天期較短的那條。四條均線都不符合但至少有 `ma20` 時顯示樸素數值，完全沒有均線資料才顯示「—」。這欄原本只看 `ma20`（單色黃底、±3% 對稱判定），2026-07-12 改版加入 `ma60`/`ma120` 並更名「甜蜜點」，`ma20Cell()` 已重新命名為 `sweetSpotCell()`；2026-07-13 加入 `ma240`，原本只有 `ma240` 用不對稱規則、其餘三條仍是 ±3%，同日再依需求把全部四條統一改成上述不對稱規則。
